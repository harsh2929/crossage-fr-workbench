# Semantic-search model pack (natural-language photo search)

On-device image+text embeddings for the `semantic_search_photos` command. Runs
fully offline on ONNX Runtime (CPU). The engine is `crossage_fr/embed/siglip_engine.py`;
it degrades gracefully (`available: false`) when these weights are absent, so the
app falls back to the lexical `search_photo_library` path.
...
- **SigLIP 2 base, patch16, 256px** — Tschannen et al. 2025, Apache-2.0.
  ...
  - Files (place all in this directory):
    ...
    | `tokenizer.json` | `tokenizer.json` | — | (Gemma tokenizer, 256k vocab) |
...
The two ONNX SHA-256 values are pinned in `crossage_fr/embed/siglip_engine.py`
(`_PINNED_SEMANTIC_HASHES`); a mismatched file is rejected (fail closed).
