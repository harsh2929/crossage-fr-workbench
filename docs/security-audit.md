...
A follow-up pass to close the documented residuals. **Every fix here was
adversarially verified** by a 4-skeptic workflow that ran real bypass attempts;
it found three holes in the first cut (all fixed + locked in as CI regressions —
`tests/mcp_redaction.py`, `tests/crypto_roundtrip.py`).

| Residual | Status | Notes |
|---|---|---|
| **USC-04** runtime model-hash re-verification | ✅ IMPLEMENTED | `.model-integrity.json` (0o600) written at download from the in-code-pinned archive hash; `verify_model_files()` fails closed at engine load — and **closes the set** (rejects an *added* unrecorded `.onnx`, the skeptic's add-a-higher-priority-file bypass). |
| **MCP-01** per-request Bearer auth | ✅ IMPLEMENTED | `streamable_http_app()` wrapped so every request needs `Authorization: Bearer <token>`; 401 before any tool. Skeptic confirmed no unauthenticated route. |
| **MCP-06** export-destination confinement | ✅ IMPLEMENTED | `_assert_allowed_path` extended to verify/restore **and** the sibling write/oracle sinks the skeptic found (`storage_io_benchmark`, `relink_workspace_paths`, dataset folder tools). |
| **MCP-04** embedded-path/basename leak | ✅ IMPLEMENTED | start-anchored redaction missed absolute paths/filenames *inside* free-text (error/audit messages); `_scrub_text()` now masks them in both resources and tool output. |
| **PC-03** at-rest encryption | ✅ IMPLEMENTED (backups) / ◑ DB | New `crossage_fr/crypto.py` (scrypt → AES-256-GCM); opt-in encrypted workspace backups via `VINTRACE_BACKUP_PASSPHRASE`. Full live-DB SQLCipher remains the larger deferred migration. |
| **BRS-3** signing/notarization | ◑ SCAFFOLDED | Entitlements + CI wiring in place; the only remaining blocker is certificate procurement (an org decision, not code). |

Implemented across commits `d0aa7d7` (security residuals), `20ee048` (PC-03).
