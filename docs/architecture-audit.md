...
**BRS-3 — Historical finding: releases shipped entirely unsigned/unnotarized; update trust rested only on TLS + `latest.yml` hashes.**
At audit time, `package.json` had no signing enforcement and both platform workflows forced `CSC_IDENTITY_AUTO_DISCOVERY=false`. *Impact:* SmartScreen/Gatekeeper warnings and no cryptographic publisher proof. **Remediated in repository policy 2026-07-13/14:** production macOS now requires Developer ID signing, notarization, stapling, and Gatekeeper checks; Windows requires Azure Artifact Signing and timestamp verification; one cross-platform finalizer verifies aggregate Sigstore/GitHub evidence before publication. Live certificate-backed artifacts remain an external release gate. See [Release Signing](release-signing.md). **Effort L.**
...
## Remaining-items implementation pass — 2026-06-14

Closed the safely-verifiable architecture deferrals. Items the audit itself
flagged as needing a runtime harness (real large-scale scan + RSS soak) or the
Electron e2e net are **deliberately left deferred** — in this environment the
e2e suite skips (no packaged build) and JS won't surface a missed global until
runtime, so doing them blind would risk *undetectable* regressions in the core
scan/IPC paths. That is a verification limit, not a scoping one.

| Item | Status | Notes / commit |
|---|---|---|
| **MA-3** extract benchmark logic | ✅ DONE | `PublicDatasetBenchmarkMixin` — verbatim AST move; api_server.py **4206 → 2989 (-29%)**; full benchmark runs end-to-end. `1a14e76` |
| **MA-1** pure-function peel | ✅ DONE (peel) / ◑ collaborators | dead `_cosine_similarity` deleted; accuracy/validation math → `match/scoring.py`, now unit-tested. Collaborator (Backup/Media/Report/Scan) split stays incremental. `4672cb2` |
| **MA-2** command registry | ✅ DONE | 96-branch if-chain → O(1) registry; every handler proven to return/raise (no fall-through regression). `d2731c1` |
| **MA-4** param schema | ◑ SEEDED | boundary required-param validation seeded from direct-index handlers; typed/range specs extend it. `d2731c1` |
| **EIPC-01** split main.cjs | ◑ FIRST SLICE | stdlib-only helpers → `desktop/main/util.cjs` (unit-tested in plain node). Stateful 8-subsystem split deferred (e2e-gated). `2f2f81b` |
| **EIPC-05** crash-loop backoff | ✅ DONE | pure capped-exponential `backendRestartDelayMs` (0ms happy path) + respawn wiring. `adc0254` |
| **CP-05** act on memory pressure | ✅ DONE | pure `memory_pressure_plan` + `gc.collect()` in the scan-polled resource monitor (no scan-loop edit). `adc0254` |
| **MS-5** case-insensitive path trust | ✅ DONE | `canonicalPathKey` case-folds on macOS/Windows; grant + check consistent. `adc0254` |
| **CP-01 / CP-07** scan off-thread + candidate streaming | ⏸ DEFERRED | XL; require a real large-library scan harness + backend-RSS soak gate before merge. |
| **CP-02 / EIPC-04** per-run cancel sentinel + stdin backpressure | ⏸ DEFERRED | need cross-process/Electron runtime verification (e2e net skips here). |
| **MS-2 / MS-4 / perf-03** | ⏸ DEFERRED | translations (MS-2), the 67-site `utcnow→aware` format change (MS-4), and ONNX batching needing perf measurement (perf-03). |
